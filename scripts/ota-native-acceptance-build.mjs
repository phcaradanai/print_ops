...
// Tauri's Windows packaging/code-generation path rewrites these tracked
// files. They are generated from the checked-out source and must be restored
// to the source commit before native provenance is recorded. Keep this list
// explicit: any other tracked change remains a hard failure.
const generatedTauriFiles = [
  'apps/desktop/src-tauri/Cargo.toml',
  'apps/desktop/src-tauri/gen/schemas/desktop-schema.json',
  'apps/desktop/src-tauri/gen/schemas/windows-schema.json',
  'apps/desktop/src-tauri/permissions/autogenerated/get_nats_settings.toml',
  'apps/desktop/src-tauri/permissions/autogenerated/save_nats_settings.toml',
  'apps/desktop/src-tauri/permissions/autogenerated/write_export_file.toml',
];
const normalizeGeneratedTauriFiles = process.env.PRINTOPS_OTA_NATIVE_NORMALIZE_GENERATED_TAURI_FILES === '1';
...
function normalizePreexistingGeneratedTauriChanges() {
  ...
  const generatedPathSet = new Set(generatedTauriFiles);
  const unexpectedPaths = changedPaths.filter((relativePath) => !generatedPathSet.has(relativePath));
  assert(
    unexpectedPaths.length === 0,
    `tracked worktree has unexpected changes for native provenance: ${unexpectedPaths.join('; ')}`,
  );
  assert(
    normalizeGeneratedTauriFiles,
    `tracked worktree contains only Tauri-generated changes; set PRINTOPS_OTA_NATIVE_NORMALIZE_GENERATED_TAURI_FILES=1 in the isolated CI job to normalize them: ${changedPaths.join('; ')}`,
  );
...
function buildInstaller({ name, version, schemaVersion, resources, includeRealServer }) {
  ...
  try {
    ...
    // Each variant build may rerun Tauri's code generation. Restore the
    // committed inputs before the next variant and before writing provenance.
    restoreGeneratedTauriFiles();
  }
}
...
function writeEvidence({ publicKey, publicKeyPath, artifacts, sourceCommit, normalizedGeneratedTauriFiles }) {
  const evidence = {
    schemaVersion: 1,
...
function main() {
  ...
  const normalizedGeneratedTauriFiles = normalizePreexistingGeneratedTauriChanges();
