...
| Git commit | `071acd4dfc4728d4138f9ddf97049be7917ed10b` |
| Installer MSI | `PrinterOps_0.1.15_x64_en-US.msi`, 26,513,408 bytes, SHA-256 `73a891fe7ea5db6141258447e2ce8296dbdf32c9e8307f5deb27ce5fee18294e` |
| Installer NSIS | `PrinterOps_0.1.15_x64-setup.exe`, 20,151,520 bytes, SHA-256 `a275cae6161093fe23b6825b6db27c30416085e98a1553d891b0f2a5cf2090cf` |
...
| --- | --- | --- |
| AUTOMATED | PASS | API 45 files/353 tests, web 23 files/453 tests, API/web typechecks and builds, Go suite, Rust suite, SQLite migration/backup, readiness, support-bundle redaction, pinned real JetStream/TCP callback matrix, 67 built-static Playwright tests, release-verifier negatives, final MSI/NSIS verification, packaged sidecar smoke, and packaged Tauri supervision smoke pass. |
...
| AUTH-03A | Migration identifies the existing OWNER without disclosing the full address | Bootstrap and conflict responses return only masked legacy OWNER hints; the UI displays them while retaining exact-email matching | `apps/api/src/tests/auth-security.test.ts`, `apps/web/src/__tests__/apiClient.test.ts` | PASS | Existing packaged database UI exercise pending |
...
| API-01 | Existing API regression suite stays green | 45 files / 353 tests pass | Local command output, 2026-07-31 | PASS | Physical and off-host integration remain |
...
| NATS-CONFIG-01 | A durable consumer conflict is diagnosed without destructive broker mutation | Test Connection and setup report the exact incompatible field, expected and actual values, and safe prefix/new-client-ID remedies; the application does not delete or retarget the durable | `apps/api/src/tests/nats-consumer-lifecycle.test.ts` | PASS | Operator must choose the authoritative production subject before changing live settings |
