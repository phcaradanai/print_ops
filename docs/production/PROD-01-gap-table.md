...
| 8. Automated release/runtime checks | PARTIAL | Real temporary JetStream, bound TCP API/callback, retry/recovery, DLQ negatives, cross-transport idempotency, migration/restart, security, lifecycle, and 67-test built-static Playwright coverage now pass. Release resource validation is executable. | Add packaged sidecar smoke and verifier negative tests, then run the final complete suite against rebuilt resources. |
